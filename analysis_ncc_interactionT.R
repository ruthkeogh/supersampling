...
# Must have interaction=T. For interaction=F use analysis_ncc_interactionF.R
#===============================
...
ncc.MIapprox.full.coef = summary_approx[,"estimate"]
ncc.MIapprox.full.se = summary_approx[,"std.error"]
...
method.vec=rep("",dim(cohort.ncc)[2])
method.vec[which(colnames(cohort.ncc)=="x")]="norm"
method.vec[which(colnames(cohort.ncc)=="xz1int")]="x*z1"

#perform the imputation
imp <- smcfcs.red(cohort.ncc, smtype="coxph", smformula="Surv(t,d)~x+z1+z2+xz1int",method=method.vec,
...
method.vec=rep("",dim(ncc.super1)[2])
method.vec[which(colnames(ncc.super1)=="x")]="norm"
method.vec[which(colnames(ncc.super1)=="xz1int")]="x*z1"

#perform the imputation
imp<-smcfcs.nestedcc.red(originaldata=ncc.super1,smformula="Surv(t.case,case)~x+z1+z2+xz1int+strata(setno)",
...
method.vec=rep("",dim(ncc.super2)[2])
method.vec[which(colnames(ncc.super2)=="x")]="norm"
method.vec[which(colnames(ncc.super2)=="xz1int")]="x*z1"

#perform the imputation
imp<-smcfcs.nestedcc.red(originaldata=ncc.super2,smformula="Surv(t.case,case)~x+z1+z2+xz1int+strata(setno)",
                           set="setno",event="case",nrisk="numrisk",
                           method=method.vec, predictorMatrix=pred.mat,
                           m=nimp,numit=nit.smc,rjlimit=10000,noisy=FALSE) 
